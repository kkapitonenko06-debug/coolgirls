...
def plot_drink_cost_structure(supplier_costs: pd.DataFrame, output_path: Path) -> None:
    ...
    _barh(
        ...
        by_category["cost_per_drink_rub"],
        output_path,
        "Структура себестоимости одного стакана",
        "Рублей на 1 напиток",
    )
...
def plot_monthly_staff_cost(staff_costs: pd.DataFrame, output_path: Path) -> None:
    ...
    _barh(
        ordered["position"],
        ordered["total_cost_rub"],
        output_path,
        "Месячная стоимость сотрудников для компании",
        "Рублей в месяц",
    )


def plot_supplier_category_coverage(coverage: pd.DataFrame, output_path: Path) -> None:
#Для этого графика сначала сделаем сводную таблицу
...
def plot_supplier_price_ranges(products: pd.DataFrame, output_path: Path) -> None:
    if products.empty:
        _empty_plot(output_path, "Цены поставщиков не собраны")
        return
    ...
    _barh(
        by_category["category"],
        by_category["median_price_rub"],
        output_path,
        "Медианная цена товаров поставщиков по категориям",
        "Рублей за товарную позицию",
    )
...
def plot_trudvsem_salary_medians(summary: pd.DataFrame, output_path: Path) -> None:
    ...
    _barh(
        ...
        ordered["median_salary_rub"],
        output_path,
        "Медианная зарплата по вакансиям Работа России",
        "Рублей в месяц",
    )
