...
#Запросы сотрудников, которые могут понадобиться
...
def collect_trudvsem_vacancies(limit: int = 100) -> pd.DataFrame:
    ...
    for target_position, query in VACANCY_SEARCH_WORDS.items():
        ...
        while True:
            try:
                ...
                print(f"Не получилось получить вакансии: {query}, offset={offset}. {error}")
                break
            ...
            offset += limit
            if not vacancies or offset >= total:
                break
            sleep(0.4)
    ...
    if not vacancies_df.empty:
        vacancies_df = vacancies_df.drop_duplicates(subset=["target_position", "vacancy_id"])
    return vacancies_df
